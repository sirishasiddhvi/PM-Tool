...
export default function ProjectLogin() {
    const [project_name, setProject_name,project_desc, setProject_Desc,client_id, setClient_Id,Manager_id, setManager_Id,err ,setErr,submit,imageChange,images, setImages,uploadwait, setUploadWait] = ProjectLoginFunction()
     ...
     return (
       <div >
        ...
        error={err == 5 && true}
        sx={{ mt: 2, width: '100%', color: '#060847', '&:hover': { color: '#060847' } }}
      >
        upload image *
        <input  type="file" multiple accept="image/jpeg, image/jpg, image/png" />
                ...
                uploadwait ? (<span style={{ color: "white" }}><br /><br />Adding Images...</span>)
                  : (<>
                    ...
                    {(images.length !== 0) ? (<CustomGrid item xs={12}>
                      <CustomGrid
                        container
                        direction="row"
                        justify="flex-start"
    ...
    <CustomGrid item>
      <Card>
        ...
        label={'DELETE IMAGE'}
          ...
          color="primary"
          component="span"
          style={{ border: "1px solid black", color: "#060847", "&:hover": { color: "#060847" } }}
          fullWidth
          onClick={() => {
